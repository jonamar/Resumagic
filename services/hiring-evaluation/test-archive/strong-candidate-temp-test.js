...
// Temperature test on strong candidate only
const temperatures = [0.1, 0.7]; // Baseline vs recommended
const candidateType = 'strong-candidate';

async function runStrongCandidateTest() {
  console.log('🧪 Strong Candidate Temperature Test');
  ...
  for (const temp of temperatures) {
    ...
    try {
      ...
      results.push({
        temperature: temp,
        candidateType,
        candidateName: candidateData.name || 'Strong Candidate',
        duration,
        overallStats,
        personaStats,
        allScores,
  ...
  if (successful.length === 2) {
    ...
    if (individualImprov > 0) {
      console.log('✅ Temperature 0.7 improves strong candidate differentiation');
    } else {
      console.log('⚠️  Temperature 0.7 may not help with strong candidates');
    }
        
    // Additional insights for strong candidates
    const temp01High = baseline.allScores.filter(s => s >= 8).length;
    const temp07High = improved.allScores.filter(s => s >= 8).length;
    console.log('\n🏆 High Scores (8+):');
    console.log(`Temp 0.1: ${temp01High}/${baseline.allScores.length} scores (${((temp01High/baseline.allScores.length)*100).toFixed(0)}%)`);
    console.log(`Temp 0.7: ${temp07High}/${improved.allScores.length} scores (${((temp07High/improved.allScores.length)*100).toFixed(0)}%)`);
  }
    
  // Save results
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const resultsFile = path.join(__dirname, `strong-candidate-temp-results-${timestamp}.json`);
...
// Run the test
runStrongCandidateTest()
  .then(() => {
    console.log('\n✅ Strong candidate temperature test completed!');
    process.exit(0);
  })
  .catch(error => {
