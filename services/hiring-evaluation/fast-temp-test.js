    ...
    try {
        ...
        evalResults.rawResults.evaluations.forEach((evaluation, index) => {
            ...
            Object.entries(evaluation.scores).forEach(([criterion, data]) => {
                ...
                if (reasoning.length < 30) qualityIssues.push(`${persona}-${criterion}: Too short (${reasoning.length} chars)`);
                if (reasoning.includes('undefined') || reasoning.includes('null')) qualityIssues.push(`${persona}-${criterion}: Contains undefined/null`);
                if (!/[.!?]$/.test(reasoning.trim())) qualityIssues.push(`${persona}-${criterion}: No proper ending punctuation`);
